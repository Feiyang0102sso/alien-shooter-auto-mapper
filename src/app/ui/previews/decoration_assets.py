...
# Stamp previews sit beside the wall previews, one folder per stamp series.
STAMP_PREVIEW_ROOT = APP_PACKAGE_ROOT / "resources" / "images" / "preview" / "AS2" / "decoration"


def get_decoration_stamp_canvas_path(series_id: str, profile_id: str) -> Path:
    """Return the transparent canvas image for one decoration stamp."""
    return STAMP_PREVIEW_ROOT / series_id / f"{profile_id}.webp"


def get_decoration_stamp_thumbnail_path(series_id: str, profile_id: str) -> Path:
    """Return the black-background UI thumbnail for one decoration stamp."""
    return STAMP_PREVIEW_ROOT / series_id / f"{profile_id}_thumbnail.webp"
...
def get_decoration_stamp_series_thumbnail_path(series_id: str) -> Path:
    ...
    return STAMP_PREVIEW_ROOT / series_id / f"{series_id}00_thumbnail.webp"
